...
(define (CRT-exists? tuplelist)
  (if (null? tuplelist)
      ...
      (if (null? (cdr tuplelist))
          #t
          (and (CRT-exists? (cdr tuplelist)) (CRThelp (cadar tuplelist) (cdr tuplelist)) ))))

(define (CRThelp int tuplelist)
  (if (null? tuplelist)
      #t
      (if (> (EuclidAlgo int (cadar tuplelist)) 1)
          #f
          (CRThelp int (cdr tuplelist)))))
