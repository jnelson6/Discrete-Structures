...
(define (union-tco s t)
  (if (null? t)
      ...
      (if (element?-tco s (car t))
          (union-tco s (cdr t))
          (union-tco (append s (list (car t))) (cdr t) ))))
...
(define (intersection-tco-help s t rv)
    (if (null? t)
      rv
      (if (element?-tco s (car t))
          (intersection-tco-help (cdr s) (cdr t) (append rv (list (car t))))
          (intersection-tco-help s (cdr t) rv))))
...
(define (set-difference-tco-help s t rv)
    (if (null? s)
      ...
      (if (element?-tco t (car s))
          (set-difference-tco-help (cdr s) (cdr t) rv)
          (set-difference-tco-help (cdr s) t (append rv (list (car s)))))))
