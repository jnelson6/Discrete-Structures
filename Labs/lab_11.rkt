...
(define (id n)
  (if (= n 0)
      '()
      (insert (append (list n) (list n)) (id(- n 1)) )
    ))
...
(define (reflexive-closure relation n)
      (union (sym-diff relation (id n))  (id n)))
...
(define (flip-pairs relation)
  (if (not(null? relation))
      (insert (reverse (car relation)) (flip-pairs (cdr relation)))
      relation))
...
(define (symmetric-closure relation)
  (if (subset? (flip-pairs relation) relation)
      relation
      (append relation (flip-pairs relation))))
...
(define (related-to element relation)
  (if (null? relation)
      ...
      (if (equal? element (car(car relation)))
          (append (list(car(cdr(car relation)))) (related-to element (cdr relation)))
